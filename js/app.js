...
function init() {
  ...
  function globalInfo(globalMarketData) {
    ...
    let marketCapChange24Value = document.querySelector(
      '.marketCapChange24Value'
    )
  ...
  function coinData(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let change1hValue =
        Math.round(data[i].price_change_percentage_1h_in_currency * 100) / 100
      let change24hValue =
        Math.round(data[i].market_cap_change_percentage_24h * 100) / 100
  ...
  function maxOrMin() {
    ...
    let listOfCoinInsideGridName = document.querySelector(
    ...
    if (this.checked) {
      ...
      for (let i = 0; i < maximalInfo.length; i++) {
        listOfCoin[i].style.gridTemplateColumns =
          '1fr 5fr 2.5fr 1.5fr 1.5fr 4fr 4fr'
